...
- Provider-backed `gpt-4o-mini` zero-shot and few-shot baselines were completed on the fixed 120-example Stage 8 sample. They are reported separately because this table is a full-test encoder and rule-baseline comparison.
...
Real zero-shot and few-shot LLM values are `n/a` in this full-test comparison table because the provider-backed run was intentionally limited to the fixed Stage 8 sample.
