...
- Blocked: no provider-backed LLM artifacts exist because no API credential was available in the workspace environment. The OpenAI Responses API path is implemented, but mock results remain excluded from scientific comparison.
...
- Real zero-shot and few-shot LLM comparisons remain blocked until a provider credential is available. No claim about instruction-following LLM performance is supported yet.
